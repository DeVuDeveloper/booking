...
RSpec.describe ReservationPolicy, type: :model do
  ...
  describe 'scopes' do
    context 'when the users asks for the reservation of the vespas' do
      let!(:vespa) { create :vespa }
      let!(:reservation_for_vespa) { create :reservation, vespa: vespa }
      let!(:vespa_2) { create :vespa }
      let!(:reservation_for_vespa_2) { create :reservation, vespa: vespa_2 }
      let(:parent_scope) { vespa.reservations }

      context 'when the user is admin' do
        let(:current_user) { create :user, admin: true }

        it 'returns all the reservations belonging to that vespa' do
          expect(actual_scope).to include(reservation_for_vespa)
        end

        it 'does not return the reservations belonging to other vespa' do
          expect(actual_scope).to_not include(reservation_for_vespa_2)
        end
      end
