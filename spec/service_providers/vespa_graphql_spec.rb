...
describe VespaGraphqlClient, pact: true do
  before do
    VespaGraphqlClient.base_uri "localhost:1234"
  end

  let(:session_token) { "token-xxx123123" }
  let(:variables) { {} }

  subject { VespaGraphqlClient.new(session_token) }
  ...
  describe "mutations" do
    describe "reservations" do
      ...
      context "when creating it" do
        let(:variables) do
          {
            "vespaId": GraphQL::Schema::UniqueWithinType.encode("Vespa", 1),
        ...
        let(:provider_state) { "a vespa exists" }
    ...
    describe "vespas" do
      context "when creating them" do
        ...
        let(:variables) do
          {
            "color": "red",
            "weight": 3.5,
            "vespaModelId": GraphQL::Schema::UniqueWithinType.encode("VespaModel", 1),
            "latitude": 23.15,
            "longitude": 35.38
          }
        end
        ...
        let(:gql) do
          <<~GQL
            mutation createBikes($color: VespaColorsEnum!, $weight: Float!, $latitude: Float!, $longitude: Float!, $vespaModelId: ID!){
        ...
        let(:message_description) { "a request for vespa creation" }
        let(:mocked_body_content) do
          {
            data: {
              createBike: {
      ...
      context "when updating them" do
        ...
        let(:gql) do
          <<~GQL
            mutation updateBike($color: VespaColorsEnum!, $weight: Float!, $vespaId: ID!){
        ...
        let(:provider_state) { "a red vespa model exists" }
        let(:message_description) { "a request for vespa update" }
        let(:mocked_body_content) do
          {
            data: {
              updateVespa: {
        ...
        it "returns the updated vespa" do
      ...
      context "when deleting it" do
        ...
        let(:gql) do
          <<~GQL
            mutation DeleteVespa($vespaId: ID!){
        ...
        let(:provider_state) { "a vespa exists" }
        let(:message_description) { "a request for vespa delete" }
        let(:mocked_body_content) do
          {
            data: {
              deleteVespa: {
        ...
        it "returns the empty errors array" do
          expect(JSON.parse(subject.delete_vespa.body, { symbolize_names: true })[:data]).to include_json(
            deleteVespa: {
              errors: []
            }
          )
  ...
  describe "queries" do
    describe "get vespa models" do
      ...
      let(:provider_state) { "a vespa model exists" }
      let(:message_description) { "a request for vespa models" }
      let(:mocked_body_content) do
        {
          data: {
            vespaModels: [{
      ...
      it "returns vespa models" do
        expect(JSON.parse(subject.get_vespa_models.body, { symbolize_names: true })[:data]).to include_json(
          vespaModels: UnorderedArray({
              id: /\d*/,
              text: /.*/,
          })
        )
    ...
    describe "get bikes" do
      context "when querying the vespas with their reservations and models" do
        let(:gql) do
          <<~GQL
            {
              vespas{
        ...
        let(:provider_state) { "a vespa with reservation exists" }
        let(:message_description) { "a request for vespa with reservations and models" }
        let(:mocked_body_content) do
          {
            data: {
              vespa: {
        ...
        it "returns vespa" do
          expect(JSON.parse(subject.get_vespas_with_reservations_and_models.body, { symbolize_names: true })[:data]).to include_json(
            vespas: {
              edges: UnorderedArray({
                node: {
                  id: /\d/,
      ...
      context "pagination" do
        ...
        let(:provider_state) { "a vespa exists" }
        let(:message_description) { "a paginated request for vespas" }
        let(:mocked_body_content) do
          {
            data: {
              vespas: {
    ...
    describe "get available vespa" do
      let(:gql) do
        <<~GQL
          query AvailableVespa{
            availableVespas(startDate: "2021-10-22", endDate:"2021-10-25"){
              nodes{
                id,
                averageRating,
                latitude,
      ...
      let(:provider_state) { "a vespa exists" }
      let(:message_description) { "a request for available vespas" }
      let(:mocked_body_content) do
        {
          data: {
            availableVespas: {
      ...
      it "returns vespas" do
        expect(JSON.parse(subject.get_available_vespas.body, { symbolize_names: true })[:data]).to include_json(
          availableVespas: {
            nodes: UnorderedArray({
              id: /\d/,
              averageRating: /\.*/,
